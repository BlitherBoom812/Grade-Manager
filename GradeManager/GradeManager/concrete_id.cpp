...
void student_id::set(const value_type &new_value) {
	...
	set_student_id(((const student_id &)new_value).id);
}
void student_id::set_id(const id_type &new_id) {
	_ASSERT(typeid(new_id) == typeid(*this));
	set_student_id(((const student_id &)new_id).id);
}
...
bool student_id::operator==(const value_type &cmp) const{
	if (typeid(cmp) != typeid(*this)) {
		...
		return id == ((const student_id &)cmp).id;
	}
}
...
bool course_id::operator<(const id_type &cmp) const {
	_ASSERT(typeid(cmp) == typeid(*this));
	const course_id &c = ((const course_id &)cmp);
...
bool course_id::operator==(const value_type &cmp) const {
	...
	const course_id &c = ((const course_id &)cmp);
	return course_first_id == c.course_first_id && course_second_id == c.course_second_id;
}
...
void grade_id::set(const value_type &new_value) {
	_ASSERT(typeid(new_value) == typeid(*this));
	const student_id &g1 = ((const student_id &)new_value);
	const course_id &g2 = ((const course_id &)new_value);
	set_student_id(g1.get_student_id());
	set_course_id(g2);
}
void grade_id::set_id(const id_type &new_id) {
	_ASSERT(typeid(new_id) == typeid(*this));
	set_student_id(((const student_id &)new_id));
	set_course_id(((const course_id &)new_id));
}
...
bool grade_id::operator==(const value_type &cmp) const {
	...
	const student_id &g1 = ((const student_id &)cmp);
	const course_id &g2 = ((const course_id &)cmp);
	return g1.get_student_id() == get_student_id() && g2.get_course_first_id() == get_course_first_id() && g2.get_course_second_id() == get_course_second_id();
}
bool grade_id::operator<(const id_type &cmp) const {
	_ASSERT(typeid(cmp) == typeid(*this));

	if (get_student_id() == ((const student_id &)cmp).get_student_id()) {
		return  (const course_id &)*this < cmp;
	}
}
const value_type &grade_id::operator=(const value_type &value) {
	_ASSERT(typeid(value) == typeid(*this));
	set_student_id((const student_id &)value);
	set_course_id(((const course_id &)value));
	return value;
}